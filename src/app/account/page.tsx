                ...
                <div className="mx-auto w-full max-w-md rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-md">
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-6">
                            <div>
                                <label className="text-xs uppercase tracking-widest text-neutral-500 font-bold">Adresse courriel</label>
                                <p className="text-lg mt-1">{user.email}</p>
                            </div>

                            <div className="flex gap-15 items-end">
                                <div>
                                    <label className="text-xs uppercase tracking-widest text-neutral-500 font-bold">Prénom</label>
                                    <p className="text-lg mt-1">{user.user_metadata.first_name}</p>
                                </div>
                                <div>
                                    <label className="text-xs uppercase tracking-widest text-neutral-500 font-bold">Nom</label>
                                    <p className="text-lg mt-1">{user.user_metadata.last_name}</p>
                                </div>
                                <button className="cursor-pointer px-5 h-full py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all group">Enregistrer</button>
                            </div>

                            <p className="text-neutral-600 text-xs">Rejoint le {new Date(user.created_at).toLocaleDateString()}</p>
                            ...
                            <div className="space-y-3">
                                ...
                                >
                                    ...
                                    <MdArrowForward className="text-neutral-500 group-hover:translate-x-1 transition-transform font-black" />
                                </Link>
